...
import './StmtToHTML.css';

import {highlightElement,
        loadLanguage} from '@speed-highlight/core';
import $ from 'jquery';
...
function highlightMetalDeviceCode() {
    const first_five_lines =
        $("#device-code-pane span:nth-child(-n + 5)");
    const has_metal_code =
        first_five_lines.get().some(
            (item) => {
                return $(item).text().search(/using namespace metal/) !== -1;
            });
    if (has_metal_code) {
        highlightElement(document.getElementById("device-code-pane"), "c", undefined, {hideLineNumbers : false});
    }
}
